...
try {
    [string]$strSBEVal = (Get-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST' -Name 'SBE').SBE #PRIVATE
} catch {
    $strSBEVal = $null
}
...
if($strSBEVal -ne '1'){
    ...
    New-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST' -Name 'SBE' -PropertyType 'String' -Value '1' -ErrorAction SilentlyContinue
    exit 0
}
if($strSBEVal -eq '1') {
