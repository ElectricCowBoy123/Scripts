...
[String]$homeFolder = "$($env:SYSTEMDRIVE)\Users\" + $($activeUser -replace '.*\\')

Start-Sleep -Seconds 2
if (-not (Test-Path "Registry::HKEY_USERS\$userSID\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST")) { #PRIVATE
  New-Item -Path "Registry::HKEY_USERS\$userSID\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST" -Force
}
...
try {
  [string]$strRDIVal = (Get-ItemProperty -Path "Registry::HKEY_USERS\$userSID\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST" -Name 'RDI').RDI
} catch {
  $strRDIVal = $null
}
...
if($strRDIVal -ne '1'){
  try {
    ...
    if(Test-Path -Path "$homeFolder\Desktop\"){
      #Write-Host "$($env:SYSTEMDRIVE)\Users\$env:USERNAME\Desktop\"
    ...
    if(Test-Path -Path "$($env:SYSTEMDRIVE)\Users\Public\Desktop\"){
      $objFiles = Get-ChildItem -Path "$($env:SYSTEMDRIVE)\Users\Public\Desktop\" -Filter "*.lnk"
      foreach($objFile in $objFiles){
        & del $objFile.FullName
      }
    }
   ...
   New-ItemProperty -Path "Registry::HKEY_USERS\$userSID\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST" -Name 'RDI' -PropertyType 'String' -Value '1' -ErrorAction SilentlyContinue
  exit 0
}
if($strRDIVal -eq '1') {
