...
try {
    [string]$strTIVal = (Get-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST' -Name 'TI').TI #PRIVATE
} catch {
    $strTIVal = $null
}
...
if($strTIVal -ne '1'){
    ...
    # Hide widgets widget (Blocked in Windows 11 24H2)
    ...
    New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST" -Name 'TI' -PropertyType 'String' -Value '1' -ErrorAction SilentlyContinue
    exit 0
}
if($strTIVal -eq '1') {
