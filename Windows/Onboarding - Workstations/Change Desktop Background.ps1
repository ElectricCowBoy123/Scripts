...
try {
    [string]$strCDBGVal = (Get-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST' -Name 'CDBG').CDBG
} catch {
    $strCDBGVal = $null
}
...
function funcChangeDesktopBG {
    ...
    try {
        New-Item -Path "$env:APPDATA" -Name "TST" -ItemType "Directory" -ErrorAction SilentlyContinue | Out-Null
    }
    catch{
    ...
    $strImage = "$env:APPDATA\TEST\tempbg.bmp"
