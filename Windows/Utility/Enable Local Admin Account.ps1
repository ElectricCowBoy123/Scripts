if ((Get-LocalUser -Name "Administrator").Enabled -eq $False) {
    try {
        ...
        # Set the user's password to never expire and enable changing password #PRIVATE
        Set-LocalUser -Name $strUsername -PasswordNeverExpires $True -Password (ConvertTo-SecureString -String $("TST-" + ((Get-WmiObject -Query 'select * from SoftwareLicensingService').OA3xOriginalProductKey -Split '-')[0]) -AsPlainText -Force)
