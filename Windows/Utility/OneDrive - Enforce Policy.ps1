...
if($null -eq $app){
    ...
    Invoke-Expression "$($env:SYSTEMDRIVE)\Users\$($strUsername)\AppData\Local\Microsoft\OneDrive\onedrive.exe /reset"
    ...
    if(Test-Path -Path "Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\OneDrive"){
        try {
            ...
            if($null -ne $(Get-ItemProperty -Path "Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\OneDrive" -Name "KFMSilentOptIn" -ErrorAction SilentlyContinue)){
                Set-ItemProperty -Path "Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\OneDrive" -Name "KFMSilentOptIn" -Value "$tenantId" -Type "String" -Force -Confirm:$false -ErrorAction Stop | Out-Null
            }
            else {
                New-ItemProperty -Path "Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\OneDrive" -Name "KFMSilentOptIn" -Value "$tenantId" -PropertyType "String" -Force -ErrorAction Stop | Out-Null
            }
        }
        catch {
