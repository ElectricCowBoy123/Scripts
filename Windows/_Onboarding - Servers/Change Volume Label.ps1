...
if($strCVLVal -ne '1'){
  try {
    Set-Volume -DriveLetter C -NewFileSystemLabel $env:COMPUTERNAME #PRIVATE
    New-ItemProperty -Path "Registry::HKEY_USERS\$strUserSID\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST" -Name 'CVL' -PropertyType 'String' -Value '1' -ErrorAction SilentlyContinue
    exit 0
  }
  catch {
    throw "[Error] Error attempting to change volume $($env:SYSTEMDRIVE)\ label`n $($_.Exception)"
  }
}
if($strCVLVal -eq '1') {
