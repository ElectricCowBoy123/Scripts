...
try {
    [string]$strCLVal = (Get-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST' -Name 'CL').CL #PRIVATE
} catch {
    $strCLVal = $null
}
...
if($strCLVal -ne '1'){
    ...
    try {
        ...
        throw "Failure Setting Timezone to GMT Standard Time"
    }

    New-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST' -Name 'CL' -PropertyType 'String' -Value '1' -ErrorAction SilentlyContinue #PRIVATE
    exit 0
}
if($strCLVal -eq '1') {
