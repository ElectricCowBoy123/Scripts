...
if (-not (Test-Path "HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST")) { #PRIVATE
...
try {
    [string]$strCDBGVal = (Get-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST' -Name 'CDBG').CDBG #PRIVATE
} catch {
    $strCDBGVal = $null
}
...
function funcChangeDesktopBG {
    ...
    try {
        New-Item -Path "$env:APPDATA" -Name "TST" -ItemType "Directory" -ErrorAction SilentlyContinue | Out-Null #PRIVATE
    }
    catch{
    ...
    $strImage = "$env:APPDATA\TEST\tempbg.bmp" #PRIVATE
...
if ($strCDBGVal -ne '1') {
    ...
    #PRIVATE
    New-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST' -Name 'CDBG' -PropertyType 'String' -Value '1' -ErrorAction SilentlyContinue
    exit 0
}
if($strCDBGVal -eq '1') {
