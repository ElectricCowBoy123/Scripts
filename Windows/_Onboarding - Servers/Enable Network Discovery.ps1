...
# Check if the registry run flag path exsts #PRIVATE
...
try { #PRIVATE
    $strENDVal = (Get-ItemProperty -Path "Registry::HKEY_USERS\$strUserSID\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST" -Name 'END').END
} catch {
    $strENDVal = $null
}
...
if ($strENDVal -ne '1') {
    ...
    #PRIVATE
    New-ItemProperty -Path "Registry::HKEY_USERS\$strUserSID\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST" -Name 'END' -PropertyType 'String' -Value '1' -ErrorAction SilentlyContinue
    exit 0
}
if($strENDVal -eq '1') {
