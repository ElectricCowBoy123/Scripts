...
$strDownloadURL = "https://SOMETHING.net/ninja/software/mspet/MSPETLaunchersetup.exe"
...
$strDesktopPath = "$($env:SYSTEMDRIVE)\Users\TEST\OneDrive - TEST\Desktop" #PRIVATE
#endregion
...
function Delete-Desktop-Icon(){
    foreach($user in (Get-ChildItem -Path "$($env:SYSTEMDRIVE)\Users\").Name) {
        foreach($dir in (Get-ChildItem -Path "$($env:SYSTEMDRIVE)\Users\$user" -Attributes H,S,D).Name){
            if($dir -eq 'Desktop'){
                foreach($file in (Get-ChildItem -Path "$($env:SYSTEMDRIVE)\Users\$user\$dir").Name){
                    if($file -eq "MSPETLauncher.lnk"){
                        Write-Host "$file Exists in '$($env:SYSTEMDRIVE)\Users\$user\$dir' deleting!"
                        Remove-Item "$($env:SYSTEMDRIVE)\Users\$user\$dir\$file" -Force
                    }
                }
            }
