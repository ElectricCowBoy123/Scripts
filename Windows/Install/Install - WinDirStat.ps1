...
$strDownloadPath = "$($env:SYSTEMDRIVE)\TEST\WinDirStat" #PRIVATE
$strURL = "https://SOMETHING.net/ninja/software/windirstat/windirstat.zip"
$strInstallDir = "$($env:SYSTEMDRIVE)\Program Files (x86)\WinDirStat"
#endregion
...
$objShortcut = $WshShell.CreateShortcut("$($env:SYSTEMDRIVE)\ProgramData\Microsoft\Windows\Start Menu\Programs\WinDirStat.lnk")
$objShortcut.TargetPath = "$strInstallDir\windirstat.exe"
$objShortcut.Save()
...
New-ItemProperty -Path $strRegistryPath -Name "UninstallString" -PropertyType ExpandString -Value "$($env:SYSTEMDRIVE)\Program Files (x86)\WinDirStat\Uninstall.exe" -Force
New-ItemProperty -Path $strRegistryPath -Name "InstallLocation" -PropertyType ExpandString -Value "$($env:SYSTEMDRIVE)\Program Files (x86)\WinDirStat" -Force
New-ItemProperty -Path $strRegistryPath -Name "DisplayName" -PropertyType String -Value "WinDirStat 1.1.2" -Force
New-ItemProperty -Path $strRegistryPath -Name "DisplayIcon" -PropertyType String -Value "$($env:SYSTEMDRIVE)\Program Files (x86)\WinDirStat\windirstat.exe,0" -Force
