...
$objDirsInC= Get-ChildItem -Path "$($env:SYSTEMDRIVE)\" -Directory
...
if (-not ($objDirsInC | Where-Object { $_.Name -eq "RedantPCS" })) {
    ...
    while($boolIsInstalled -eq $False){
        ...
        if (($objDirsInC | Where-Object { $_.Name -eq "RedantPCS" })) {
            ...
            Write-Host "Copying RedAntPCS.txt to $($env:SYSTEMDRIVE)\RedantPCS..."
            try{
                Copy-Item -Path "\\$env:shareServer\$env:shareName\Install\RedAntPCS.txt" -Destination "$($env:SYSTEMDRIVE)\RedantPCS\RedAntPCS.txt" -Force
            }
            catch {
                throw "[Error]: Failed to copy configuration file from \\$env:shareServer\$env:shareName\Install\RedAntPCS.txt to $($env:SYSTEMDRIVE)\RedantPCS\RedAntPCS.txt Details: $($_.Exception)"
            }
            ...
            try {
                ...
                $desktopPath = "$($env:SYSTEMDRIVE)\Users\Public\Desktop"
            ...
            try{& regsvr32 /s "$($env:SYSTEMDRIVE)\RedantPCS\c1sizerppg.dll"}catch{throw "Failed to register c1sizerppg.dll"}
            try{& regsvr32 /s "$($env:SYSTEMDRIVE)\RedantPCS\c1Sizer.ocx"}catch{throw "Failed to register c1Sizer.ocx"}
            try{& regsvr32 /s "$($env:SYSTEMDRIVE)\RedantPCS\Codejock.SkinFramework.v15.3.1.ocx"}catch{throw "Failed to register Codejock.SkinFramework.v15.3.1.ocx"}
            try{& regsvr32 /s "$($env:SYSTEMDRIVE)\RedantPCS\emsmtp.dll"}catch{throw "Failed to register emsmtp.dll"}
            try{& regsvr32 /s "$($env:SYSTEMDRIVE)\RedantPCS\emssl.dll"}catch{throw "Failed to register emssl.dll"}
            try{& regsvr32 /s "$($env:SYSTEMDRIVE)\RedantPCS\ImageViewer2.ocx"}catch{throw "Failed to register ImageViewer2.ocx"}
            try{& regsvr32 /s "$($env:SYSTEMDRIVE)\RedantPCS\ScPro.ocx"}catch{throw "Failed to register ScPro.ocx"}
            try{& regsvr32 /s "$($env:SYSTEMDRIVE)\RedantPCS\viscomgifenc.dll"}catch{throw "Failed to register viscomgifenc.dll"}
