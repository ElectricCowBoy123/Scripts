...
try {
    [string]$strCLVal = (Get-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST' -Name 'CL').CL
} catch {
    $strCLVal = $null
}
...
if($strCLVal -ne '1'){
    ...
    New-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon\TST' -Name 'CL' -PropertyType 'String' -Value '1' -ErrorAction SilentlyContinue
    exit 0
}
if($strCLVal -eq '1') {
