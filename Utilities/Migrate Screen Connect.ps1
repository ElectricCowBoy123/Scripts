...
function funcUninstallApplication {
    param (
        [Parameter(Mandatory = $True)]
        [String]$UninstallString
    )
    # Execute the uninstall command
    Write-Host "[Informational] Uninstalling application..."
    $process = Start-Process -FilePath "msiexec.exe" -ArgumentList "/x $UninstallString /qn /norestart" -Wait -PassThru
    Write-Host "[Informational] Uninstall Exit Code: $($process.ExitCode)"
    
    if ($process.ExitCode -ne 0) {
        throw "[Error] Uninstallation failed with exit code: $($process.ExitCode)"
    }
}
...
function funcGetControlPanelName {
    ...
    if (-not (Test-Path $strMSIPath)) {
        throw "[Error] The MSI file does not exist at path '$strMSIPath'."
    }

    $objWindowsInstaller = New-Object -ComObject WindowsInstaller.Installer
    $objDatabase = $objWindowsInstaller.OpenDatabase($strMSIPath, 0)

    if ($null -eq $objDatabase) {
        throw "[Error] Failed to open the MSI database."
    }

    $strQuery = "SELECT `Value` FROM `Property` WHERE `Property` = 'ProductName'"
    $objView = $objDatabase.OpenView($strQuery)
    $objView.Execute()

    $objRecord = $objView.Fetch()
    ...
    if ($objRecord) {
        return $objRecord.StringData(1)
    } else {
        throw "[Error] Failed to fetch the product name from the MSI."
    }
}
...
function funcInvokeDownload {
    ...
    While ($i -lt 4) {
        ...
        if ($File) {
            Write-Host "[Informational] Download successful: $strPath"
            return
        }
        else {
            Write-Host "[Warning] Download failed, retrying..."
            $i++
        }
    }

    throw "[Error] Failed to download file after multiple attempts!"
}
...
if ($boolIsInstalled) {
    Write-Host "[Informational] $objUserroductName is already installed; uninstalling..."
    $uninstallString = $boolIsInstalled[0].UninstallString
    Write-Host "[Informational] Uninstall String: '$uninstallString'"
    funcUninstallApplication -UninstallString $uninstallString
}
